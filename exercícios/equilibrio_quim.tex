...
\begin{tikzpicture}[scale=0.5]
	...
	\end{tikzpicture}
				...
				\question Considere a seguinte equação de oxi-redução:
				\begin{center}
					\ce{Ce^{4+} + Fe^{2+} <--> Ce^{3+} + Fe^{3+}}
				\end{center}
				A constante de equilíbrio desta reação é igual a $2,0 \times 10^{11}$.
				\begin{enumerate}[label=\alph*)]
				\item Escreva a expressão que representa a constante de equilíbrio.
					\makeemptybox{2cm}
				\item calcule a concentração de íons \ce{Ce^{4+}} que existe em equilíbrio em uma solução cuja a concentração de \ce{Ce^{3+}} é 0,1 mol/L, de \ce{Fe^{3+}} é 0,1 mol/L e de \ce{Fe^{2+}} é 0,1 mol/L.
					 \makeemptybox{2cm}
				\end{enumerate}

				\question Dois mol de \ce{CO_{(g)}} reagem com dois mol de \ce{NO2_{(g)}}, conforme a equação.
				\begin{center}
					\ce{CO_{(g)} + NO2_{(g)} <-->[1][2] CO2_{(g)} + NO_{(g)}} ($200^{\circ}C$)
				\end{center}
				Quando se estabelece o equilíbrio, verifica-se que \sfrac{3}{4} de cada um dos reagentes foram transformados em \ce{CO2_{(g)}} e \ce{NO_{(g)}}. A constante de equilíbrio para a reação é:
				\makeemptybox{2cm}
				...
				\question O equilíbrio gasoso \ce{N2O4 <--> 2 NO2} apresenta, a uma dada temperatura, constante de equilíbrio $K_C$ = 2. Nesta temperatura foram feitas duas misturas diferentes, A e B, cada uma acondicionada em recipiente fechado, isolado e distinto. As condições iniciais estão mostradas abaixo:
				\begin{enumerate}
				\item mistura A \ce{NO2} = $2\times 10^{-2}$ mol/L \ce{N2O4} = $2\times 10^{-4}$ mol/L
				\item mistura B \ce{NO2} = $2\times 10^{-1}$ mol/L \ce{N2O4} = $1\times 10^{-3}$ mol/L
					\end{enumerate}
				\begin{enumerate}[label=\alph*.]
